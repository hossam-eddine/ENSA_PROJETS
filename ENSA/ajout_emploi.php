...
$req=" Insert Into horaires Values('" .$_POST['HD']. "','" .$_POST['HF']. "','" .$_POST['ens']. "','" .$_POST['codeG']. "','" .$_POST['MT']. "','" .$_POST['codeL']. "','" .$_POST['jour']. "' )";

$dbprep=$conn->prepare($req);
$dbexecut=$dbprep->execute();
if($dbexecut){
  echo "<script>alert('Nouveau Seance ajouter');
  location.href = 'add_emploi.php';
  </script>";
...
<!--------enseignant---!-->
  ...
  <th bgcolor="#40ACB5"><strong>Matiere</strong></th>
    ...
    <th  bgcolor="#40ACB5"><strong>Groupe</strong></th>
...
<select id="codeL" name="codeL" required>
  <option value=""></option>
  <?php
$sql = "SELECT DISTINCT(codeL) FROM locaux ,groupes where  locaux.codeD=groupes.codeD and groupes.semestre='".$_POST["annee"]."'and groupes.codeD='".$_POST["codeD"]."' ";
$sql_result = $conn->query($sql);
while ($row = $sql_result->fetch()) {
...
<select id="codeG" name="codeG" required>
  <option value=""></option>
  <?php
$sql = "SELECT * from groupes where groupes.semestre= '".$_POST["annee"]."'and groupes.codeD='".$_POST["codeD"]."'";
$sql_result = $conn->query($sql);
while ($row = $sql_result->fetch()) {
echo "<option value='".$row["0"]."'".">".$row["0"]."</option>";
}
